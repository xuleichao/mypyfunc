...
def gamma_i_j_cal(x, mu, sigama, weight_lst, k):
    # 全概率计算
    P_sum = 0
    sub_prob = []
    for i in range(len(weight_lst)):
        P = P_of_mu_sigama(x, mu, sigama)
...
def new_mu_vector_cal(datasets, mu, sigama, k, weight_lst):
    '''
    - k: 第几个高斯分量
    '''
    result = {}
...
def new_sigama_cal(datasets, mu, sigama, k, weight_lst):
    ...
    for _ in range(len(weight_lst)):
        ...
        result[str(i) + '_sum_sigama_ij_x'] = np.zeros((len(weight_lst), len(weight_lst)))
        break
    for i in datasets:
        gamma_i = gamma_i_j_cal(i, mu, sigama, weight_lst, k)
        for _ in range(len(weight_lst)):
...
if __name__ == '__main__':
    ...
    # 参数初始化
    # 平均值
    mu_lst = [np.array(datasets[0]), np.array(datasets[10])]
    # 协方差
    sigama_lst = [np.array([[1, 0],
                           [0, 1.9]], dtype='float64'),
                 np.array([[1, 0],
                           [0, 1]], dtype='float64')]
    # 高斯的权重
    ...
    for i in range(num):
        ...
        for j in range(len(mu_lst)):
            ...
            sigama = sigama_lst[j]
            weight = weight_lst[j]
            new_mu = new_mu_vector_cal(datasets, mu, sigama, j, weight_lst)
            new_sigama = new_sigama_cal(datasets, mu, sigama, j, weight_lst)
            new_weight = new_weight_cal(datasets, mu, sigama, j, weight_lst)
